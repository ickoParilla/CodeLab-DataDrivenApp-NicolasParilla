...
string ofApp::boxedString(string text, int width) {
	string boxedStringTemp1 = "";
	string boxedStringTemp2 = "";
	vector <string> wordsTemp = ofSplitString(text, " ");

	for (int i = 0; i < wordsTemp.size(); i++) {
		string tweetTemp = wordsTemp[i]; // Gets data for tweetTemp

		if (i > 0) { // Adds a space after every word
			boxedStringTemp2 += " ";
		}

		boxedStringTemp2 += tweetTemp; //Adds the current word in tweetTemp to boxedStringTemp2

		int stringwidth = font.stringWidth(boxedStringTemp2); // Sets strings width to match with sentence length

		if (stringwidth >= width) { // checks string width to check wether space or new is required
			boxedStringTemp1 += "\n"; // if line is out the box size, adds a new line
			boxedStringTemp2 = tweetTemp; // makes sure that it's including the extra word on to the next line
		}

		else if (i > 0) {
			boxedStringTemp1 += " "; // Adds space between the words
		}
		boxedStringTemp1 += tweetTemp; // Adds current word to string to be returned with new lines for spacing
	}

	return boxedStringTemp1;
